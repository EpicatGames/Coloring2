...
{
    public class _PagesSwiper : MonoBehaviour, IDragHandler, IEndDragHandler
    {
        ...
        public void AddPages<T>(List<T> pages) where T : IPage
        {
            ...
            {
                var page = pages[i];
                page.RectTransform.SetParent(_content);
                Pages.Add(page);
                page.Init(i);
            }
        ...
        {
            page.RectTransform.SetParent(_content);
            page.RectTransform.SetAsLastSibling();
            page.Init(page.RectTransform.GetSiblingIndex());
            Pages.Add(page);
            TotalPages = _content.childCount;
        }
        ...
        public void AddPagesPrefabs<T>(List<T> pagesPrefabs) where T : IPage
        {
            for (var i = 0; i < pagesPrefabs.Count; i++)
            {
                ...
                var page = Instantiate(pageRef.RectTransform.gameObject, _content).GetComponent<IPage>();
                Pages.Add(page);
                page.Init(i);
            }
            TotalPages = _content.childCount;
        ...
        {
            var page = Instantiate(pagePrefab.RectTransform.gameObject, _content).GetComponent<IPage>();
            page.RectTransform.SetAsLastSibling();
            page.Init(page.RectTransform.GetSiblingIndex());
            TotalPages = _content.childCount;
        }
        ...
        public void ShowPage(int id)
        {
            ...
            var newX = Pages[id].GetSize().x * id;
        ...
        public void NextPage()
        {
            ...
            var offset = currentPage.GetSize().x;
            int nextID;
            ...
            else
            {
                if (GetLastPage() == currentPage)
                {
                    ...
                    nextID = firstPage.Id;
                    var firstTransform = (RectTransform)firstPage.RectTransform;
                    var currentTransform = (RectTransform)currentPage.RectTransform;
        ...
        public void PrevPage()
        {
            ...
            else
            {
                if (GetFirstPage() == currentPage)
                {
                    ...
                    prevID = lastPage.Id;
                    var lastTransform = (RectTransform)lastPage.RectTransform;
                    var currentTransform = (RectTransform)currentPage.RectTransform;
        ...
        public void OnDrag(PointerEventData data)
        {
            ...
            var firstTransform = (RectTransform)first.RectTransform;
            var lastTransform = (RectTransform)last.RectTransform;
            
            if (GetRectLeftTopWorldCoord(firstTransform) > 0)
            {
                lastTransform.anchoredPosition = firstTransform.anchoredPosition - new Vector2(last.GetSize().x, 0);
                return;
            }
            ...
            if (GetRectLeftTopWorldCoord(lastTransform) < Math.Abs(lastTransform.rect.x * .5f))
                firstTransform.anchoredPosition = lastTransform.anchoredPosition + new Vector2(last.GetSize().x, 0);
        ...
        private IPage GetFirstPage()
        {
            ...
            {
                if (!(page.RectTransform.position.x < min)) 
                    continue;
                min = page.RectTransform.position.x;
                result = page;
            }
            return result;
        }
        ...
        private IPage GetLastPage()
        {
            ...
            foreach (var page in Pages)
            {
                if (!(page.RectTransform.position.x > max))
                    continue;
                max = page.RectTransform.position.x;
                result = page;
            }
            return result;
        }
